...
public class AddCreditCardView extends AbstractEntityView {
    ...
    public AddCreditCardView(Entity entity, Node viewNode) {
        ...
        Label headerLabel = new Label("주소 수정", "NewCreditCardHeaderLabel");
        ...
        TextField month = new TextField("", "월", 20, TextArea.NUMERIC);
        month.setUIID("NewCardField");
        month.getHintLabel().setUIID("NewCardFieldHint");

        TextField year = new TextField("", "년", 20, TextArea.NUMERIC);
        ...
        wrapper.add(BorderLayout.CENTER, BoxLayout.encloseY(
                new Label("카드 소유자 이름", "AddNewCardExplanation"),
                name,
                new Label("신용카드 번호", "AddNewCardExplanation"),
                GridLayout.encloseIn(4, num1, num2, num3, num4),
                new Label("만료일", "AddNewCardExplanation"),
                GridLayout.encloseIn(2, month, year)
        ));

        Button saveCard = new Button("카드 저장", "SaveCardConfirmButton");
        validator.addSubmitButtons(saveCard);
        saveCard.addActionListener(evt -> {
            evt.consume();
            Entity creditCard = new CreditCardModel(
                    name.getText(),
                    num1.getText() + num2.getText() + num3.getText() + num4.getText(),
                    year.getText(),
                    month.getText()
            );
