...
public class HomeView extends AbstractEntityView {
    ...
    public HomeView(Entity appEntity, Node viewNode) {
        this(appEntity, viewNode, "");
    }

    public HomeView(Entity appEntity, Node viewNode, String selectedCategory) {
        super(appEntity);
...
// ✅ 사용자 이름 구성
        String firstName = accountModel.getText(com.codename1.demos.grub.interfaces.Account.firstName);
        String lastName = accountModel.getText(com.codename1.demos.grub.interfaces.Account.lastName);
        ...
        if (username.isEmpty()) {
            username = "고객"; // 기본 이름 처리
        }

// 주소 표시 UI 설정
        deliverToLabel = new Label("", "HomeDeliverToLabel");
        deliverToCnt = BoxLayout.encloseY(new Button("수강 장소", "HomeDeliverToHeaderButton"), deliverToLabel);
...
// 최종 UI 조합
        Container topViewLabelsCnt = BorderLayout.north(BoxLayout.encloseY(
                deliverToCnt,
                new Label(welcomeText, "HomeWelcomeTextFirstLine"),
                new Label(username + "님!", "HomeWelcomeTextSecondLine")  // ← 사용자 이름 표시!
        ));

        topView.add(topViewLabelsCnt);
        add(topView);

        Container categoryRiceButton = createCategoryButton("코딩", "rice-icon.png", evt -> {
        ...
        Container categoryRiceButton1 = createCategoryButton("운동", "pizza-icon.png", evt -> {
        ...
        Container categoryRiceButton2 = createCategoryButton("음악", "donut-icon.png", evt -> {
        ...
        Container categoryRiceButton3 = createCategoryButton("요리", "chicken-icon.png", evt -> {
        ...
        Container categoryRiceButton4 = createCategoryButton("입시", "steak-icon.png", evt -> {
        ...
        Container categoryRiceButton5 = createCategoryButton("디자인", "meal-icon.png", evt -> {
        ...
        Container categoryRiceButton6 = createCategoryButton("게임", "kebab-icon.png", evt -> {
    ...
    private Container createAllRestaurantsCnt(EntityList<Entity> restaurants) {
        ...
        for (Entity rest : restaurants) {
            if (selectedCategory == null || selectedCategory.isEmpty() ||
                    rest.getText(Restaurant.category).equals(selectedCategory)) {
                allRestsCnt.add(new RestaurantPreview(rest, viewNode));
            }
        }
